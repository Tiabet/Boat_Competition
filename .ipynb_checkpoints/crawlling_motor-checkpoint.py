...
def crawl_motor_info(year):
    base_url = f'https://www.kboat.or.kr/contents/information/raceRecordList.do?repairCode=M&searchYear={year}'
    ...
    for page_num in tqdm(range(1, max_page_num + 1)):
        ...
        for row in rows:
            ...
            # 데이터를 리스트에 추가
            all_motor_data.append([Year, Motor_No, 출전회차, 출주회수, 일착, 이착, 삼착, 평균착순점, 연대율, 삼주회, 이주회_온라인, 이주회_플라잉])

    # Pandas DataFrame으로 변환
    columns = ['Year', 'Motor_No', '출전회차', '출주회수', '1착', '2착', '3착', '평균착순점', '연대율', '3주회', '2주회(온라인)', '2주회(플라잉)']
    df = pd.DataFrame(all_motor_data, columns=columns)

    # CSV 파일로 저장
    df.to_csv(f'./crawlled_data/kboat_motor_{year}.csv', index=False, encoding='utf-8-sig')
    print(f'{year}년도 모터 정보 크롤링 완료, motor_info_{year}.csv로 저장되었습니다.')
